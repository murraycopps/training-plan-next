...
function getDays(text: string) {
    ...
    for (const line of lines) {
        if (line) {
            ...
            if (day && workout) {
                const [workoutType] = workout.split(' ');
                if (workoutType === 'workout' || workoutType === 'track' || workoutType === "hills" || workoutType === "hill") {
...
export default function TrainingForm({ onSubmit, savedDays }: { onSubmit: Dispatch<SetStateAction<Day[]>>, savedDays: SavedDays[] }) {
    ...
    const handleSubmit = async (event: { preventDefault: () => void }) => {
        ...
        if (!experience || !age || !normalMilage || !targetRace || !workouts) return alert('Please fill out all fields')
        ...
        try {
            ...
            const totalMileage = days.reduce((acc, day) => acc + (day.mileage || 0), 0);

            if (totalMileage > parseInt(normalMilage) * 1.2 || totalMileage < parseInt(normalMilage) * 0.8) {
                console.log(`totalMileage is ${totalMileage} and normalMilage is ${normalMilage}`)
                handleSubmit(event)
            }
    ...
    return (
        <>
            <form onSubmit={handleSubmit} className="bg-gray-600 shadow-2xl rounded-lg px-8 pt-6 pb-6 mb-4">
                {!loaded && (
                    <div className="bg-gray-300 h-full w-full absolute top-0 left-0 z-50">
                        <div className="flex flex-col items-center justify-center h-full w-full">
                            ...
                            <p className="text-xl font-bold mt-4 text-center text-gray-600">Loading...</p>
                        </div>

                    </div>
                )}
                <h1 className="text-4xl font-bold text-center mb-4">Generate Training Plan</h1>
                <div className="mb-4">
                ...
                <div className="flex items-center justify-between">
                    <button
                        className="hover:bg-gray-800 bg-gray-400 text-gray-600 hover:text-gray-300 w-full mx-16 mt-4 py-4 rounded-lg" type="submit">
                        Submit
                    </button>
                </div>
            </form>
